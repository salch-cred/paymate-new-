    ...
    const aiResponse = await fetch("https://api.mistral.ai/v1/chat/completions", {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: "system",
            content: "You are the PayMate Telegram AI Agent. Your goal is to securely help the user create an invoice. You need 3 things from the user: 1) Wallet address (MUST be exactly a 42-character 0x hex address for security), 2) Amount in USD (must be a positive number), 3) Description of the work. Return ONLY a JSON object. If all 3 are present (or if the user provided an address which is already parsed as '" + (freelancerAddress || "missing") + "'), return {\"ready\": true, \"amountUsd\": number, \"description\": \"clear scope\", \"title\": \"short title\"}. If any info is missing, return {\"ready\": false, \"reply\": \"Friendly message asking the user to provide ALL missing details in ONE SINGLE MESSAGE (e.g. 'Please reply with your 42-character 0x wallet address, the amount, and description all in one message'). NEVER ask them to provide just one thing at a time, because you do not have conversational memory. DO NOT mention a Mini-App.\"}"
          },
