...
export default function DashboardPage(){
 ...
 const createInvoiceMutation=useMutation({mutationFn:async()=>{const splitsPayload=splits.length>0?splits.map(s=>({address:s.address,amountUsd:Number(s.amountUsd)})):undefined;const res=await fetch(`/api/invoices`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({freelancer:address,client,title,description,amountUsd:Number(amount),dueDate:dueDate||null,splits:splitsPayload})});const data=await res.json();if(!res.ok)throw new Error(data.detail||"Invoice creation failed");return {...data.invoice,payUrl:`${location.origin}${data.payUrl}`} as Invoice},onSuccess:(created)=>{queryClient.setQueryData<Invoice[]>(["invoices",address],(prev)=>[created,...(prev??[])]);setClient("");setTitle("");setDescription("");setAmount("");setDueDate("");setSplits([]);setFormError(null)},onError:(e)=>setFormError(e instanceof Error?e.message:"Could not create invoice")})
 ...
 function createInvoice(e:React.FormEvent){e.preventDefault();if(!address)return setFormError("Connect your wallet first.");if(!isAddress(client))return setFormError("Enter a valid client wallet address.");if(!title.trim()||description.trim().length<5||!amount||Number(amount)<=0)return setFormError("Complete the title, work description, and amount.");if(splits.length>0){const splitTotal=splits.reduce((sum,s)=>sum+Number(s.amountUsd),0);if(Math.abs(splitTotal-Number(amount))>0.01)return setFormError(`Total of splits ($${splitTotal}) must equal total invoice amount ($${amount}).`)}createInvoiceMutation.mutate()}
 ...
 <div style={{padding:'0 16px 16px'}}>
  <button type="button" className="button button-dark" style={{width:'100%',justifyContent:'center'}} disabled={zkLoading} onClick={()=>{setZkLoading(true);setTimeout(()=>{setZkProof(`0x${Math.random().toString(16).substring(2,10)}...${Math.random().toString(16).substring(2,10)}`);setZkLoading(false)},2000)}}>{zkLoading?<><span className="draft-spinner"/>Generating Proof…</>:<><Icon name="lock" size={14}/>Generate ZK-Proof</>}</button>
  {zkProof&&<div style={{marginTop:'12px',padding:'12px',background:'rgba(255,255,255,0.4)',borderRadius:'8px',fontSize:'11px',wordBreak:'break-all',fontFamily:'monospace'}}><b>Proof Hash:</b><br/>{zkProof}</div>}
 </div>
