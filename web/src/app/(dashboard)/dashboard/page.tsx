...
const [createdInvoice, setCreatedInvoice] = useState<(Invoice & { payUrl: string }) | null>(null)
 const [formError,setFormError]=useState<string|null>(null);const [copied,setCopied]=useState<string|null>(null);const [expandedId,setExpandedId]=useState<string|null>(null)
 ...
 const res=await fetch(`/api/invoices`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({freelancer:address,client,title,description,amountUsd:calculatedAmount,dueDate:dueDate||null,splits:splitsPayload,milestones:msPayload,isPrivate:privacyMode,zkCommitment})});const data=await res.json();if(!res.ok)throw new Error(data.detail||"Invoice creation failed");return {...data.invoice,payUrl:`${location.origin}${data.payUrl}`,zkViewKey} as Invoice&{payUrl:string;zkViewKey:string|null}},onSuccess:(created)=>{queryClient.setQueryData<Invoice[]>(["invoices",address],(prev)=>[created,...(prev??[])]);setCreatedInvoice(created);if(created.zkViewKey){setZkViewKeys(prev=>({...prev,[created.id]:created.zkViewKey as string}))}setClient("");setTitle("");setDescription("");setAmount("");setDueDate("");setSplits([]);setMilestones([]);setFormError(null)},onError:(e)=>setFormError(e instanceof Error?e.message:"Could not create invoice")})
 ...
 function invoiceLink(inv:Invoice){const vk=zkViewKeys[inv.id];return `${location.origin}/pay/${inv.id}`+(vk?`#key=${vk}`:"")}
 async function copyInvoice(inv:Invoice){await navigator.clipboard.writeText(invoiceLink(inv));setCopied(inv.id);setTimeout(()=>setCopied(null),1500)}
 async function copyInvoiceId(inv:Invoice){await navigator.clipboard.writeText(inv.id);setCopiedId(inv.id);setTimeout(()=>setCopiedId(null),1500)}
 ...
 const q=search.trim().toLowerCase();const filteredInvoices=q?invoices.filter(inv=>[inv.title,inv.id,inv.client,inv.txHash].some(f=>f?.toLowerCase().includes(q))):invoices
  return (
    <>
      ...
      <div className="workspace-toolbar"><div className="toolbar-search"><Icon name="spark" size={15}/><input ref={searchRef} value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search invoices, clients, or transaction hashes"/><kbd>⌘ K</kbd></div><div className="toolbar-status"><span><i/>API ONLINE</span><span><i/>GOAT LIVE</span></div><a href="#new" className="toolbar-create"><Icon name="invoice" size={15}/>New invoice</a></div><div className="dashboard-journey"><div className="journey-progress"/><span className="active"><i>1</i><b>Draft</b><small>Structure the work</small></span><em><Icon name="arrow" size={13} /></em><span><i>2</i><b>Review</b><small>Approve every detail</small></span><em><Icon name="arrow" size={13} /></em><span><i>3</i><b>Share</b><small>Send one link</small></span><em><Icon name="arrow" size={13} /></em><span><i>4</i><b>Verify</b><small>Settle on GOAT</small></span></div>
 ...
 <div><section className="panel panel-pad" id="new"><div className="panel-heading"><div><h2>Create a verified invoice</h2><p>Clear terms, direct payment, permanent proof.</p></div><span className="icon-box"><Icon name="spark"/></span></div><div className="smart-drafter"><div className="smart-drafter-head"><div><span><Icon name="spark" size={15}/>INTELLIGENT DRAFTING</span><p>Describe the work naturally or use the Voice AI Agent.</p></div>{draftMeta&&<small className="draft-confidence"><i/>{draftMeta.source==="ai-voice"?"Voice AI structured":draftMeta.source==="ai"?"AI structured":"Safe parser"} · {Math.round(draftMeta.confidence*100)}%</small>}</div><div className="smart-drafter-input"><textarea value={draftPrompt} onChange={e=>setDraftPrompt(e.target.value)} placeholder="Example: Brand strategy and visual identity... Total agreed: $2,480 USDC..."/><div style={{display:'flex',flexDirection:'column',gap:'8px'}}><button type="button" onClick={startListening} style={{height:'34px',borderRadius:'11px',border:'none',background:isListening?'#ff5b2e':'rgba(23,24,19,0.05)',color:isListening?'white':'var(--ink)',display:'flex',alignItems:'center',justifyContent:'center',gap:'6px',padding:'0 12px',cursor:'pointer',boxShadow:isListening?'0 0 0 4px rgba(255,91,46,0.2)':'none',transition:'0.2s',fontSize:'10px',fontWeight:700,whiteSpace:'nowrap'}}><Icon name="spark" size={13}/> {isListening ? "Listening..." : "Voice Agent"}</button><button type="button" onClick={generateDraft} disabled={draftMutation.isPending}>{draftMutation.isPending?<><span className="draft-spinner"/>Structuring…</>:<><Icon name="spark" size={14}/>Generate draft</>}</button></div></div>{draftMeta&&<div className="draft-review-note"><Icon name="shield" size={14}/><span>Draft filled below. Review the amount and terms before creating the payment link.</span>{draftMeta.paymentTerms&&<b>{draftMeta.paymentTerms}</b>}</div>}</div>{createdInvoice&&<div style={{marginTop:'14px',padding:'12px 14px',borderRadius:'12px',background:'rgba(49,130,93,0.08)',border:'1px solid rgba(49,130,93,0.3)'}}><div style={{display:'flex',alignItems:'center',gap:'8px',marginBottom:'8px'}}><Icon name="check" size={16} style={{color:'#31825d',flexShrink:0}}/><b style={{fontSize:'12px'}}>Invoice created — share it with your client</b><button type="button" onClick={()=>setCreatedInvoice(null)} aria-label="Dismiss" style={{marginLeft:'auto',background:'transparent',border:'none',cursor:'pointer',color:'var(--muted)'}}><Icon name="close" size={14}/></button></div><div style={{display:'flex',alignItems:'center',gap:'8px',padding:'8px 10px',background:'rgba(255,255,255,0.5)',border:'1px solid var(--line)',borderRadius:'8px',marginBottom:'6px'}}><span style={{fontSize:'10px',fontWeight:800,textTransform:'uppercase',letterSpacing:'0.06em',color:'var(--muted)',flexShrink:0}}>Invoice ID</span><code style={{flex:1,minWidth:0,fontSize:'11px',wordBreak:'break-all'}}>{createdInvoice.id}</code><button type="button" onClick={()=>copyInvoiceId(createdInvoice)} style={{display:'flex',alignItems:'center',gap:'5px',background:'transparent',border:'1px solid var(--line)',borderRadius:'8px',padding:'5px 9px',fontSize:'11px',fontWeight:700,cursor:'pointer',whiteSpace:'nowrap',flexShrink:0}}><Icon name={copiedId===createdInvoice.id?"check":"copy"} size={13}/>{copiedId===createdInvoice.id?"Copied":"Copy"}</button></div><div style={{display:'flex',alignItems:'center',gap:'8px',padding:'8px 10px',background:'rgba(255,255,255,0.5)',border:'1px solid var(--line)',borderRadius:'8px'}}><span style={{fontSize:'10px',fontWeight:800,textTransform:'uppercase',letterSpacing:'0.06em',color:'var(--muted)',flexShrink:0}}>Pay link</span><code style={{flex:1,minWidth:0,fontSize:'11px',wordBreak:'break-all'}}>{createdInvoice.payUrl}</code><button type="button" onClick={()=>copyInvoice(createdInvoice)} style={{display:'flex',alignItems:'center',gap:'5px',background:'transparent',border:'1px solid var(--line)',borderRadius:'8px',padding:'5px 9px',fontSize:'11px',fontWeight:700,cursor:'pointer',whiteSpace:'nowrap',flexShrink:0}}><Icon name={copied===createdInvoice.id?"check":"link"} size={13}/>{copied===createdInvoice.id?"Copied":"Copy"}</button></div><small style={{display:'block',marginTop:'8px',fontSize:'11px',color:'var(--muted)'}}>Client can pay by typing the ID at /pay — or just open the link.</small></div>}<form onSubmit={createInvoice}><div className="form-grid"><div className="field"><label>Client wallet</label><input value={client} onChange={e=>setClient(e.target.value)} placeholder="0x..."/></div><div className="field"><label>Project title</label><input value={title} onChange={e=>setTitle(e.target.value)} placeholder="Brand system sprint"/></div><div className="field"><label>Amount · USDC</label><input type="number" min="0.01" step="0.01" value={milestones.length>0?milestones.reduce((s,m)=>s+Number(m.amountUsd),0):amount} disabled={milestones.length>0} onChange={e=>setAmount(e.target.value)} placeholder="2480.00"/></div><div className="field"><label>Due date</label><input type="date" value={dueDate} onChange={e=>setDueDate(e.target.value)}/></div><div className="field full"><label>Scope delivered</label><textarea value={description} onChange={e=>setDescription(e.target.value)} placeholder="Describe the completed work, included deliverables, and agreed terms."/></div></div>
 ...
 <section className="panel activity-panel" id="activity"><div className="panel-heading"><div><h2>Invoice activity</h2><p>Live status from creation to verified settlement.</p></div><span className="activity-count">{filteredInvoices.length}</span></div>{invoices.length===0?<div className="activity-empty">Your first invoice will appear here.</div>:filteredInvoices.length===0?<div className="activity-empty">No invoices match &quot;{search}&quot;.</div>:<div className="invoice-table">{filteredInvoices.map(inv=><div className="invoice-item" key={inv.id}><div className="invoice-row"><span className={`status-dot ${inv.status}`}/><div className="invoice-row-main"><b>{inv.title}</b><small>{new Date(inv.createdAt).toLocaleDateString()} · {inv.id.slice(0,8)}</small></div><strong>${inv.amountUsd.toLocaleString()}</strong><span className={`status-label ${inv.status}`}>{inv.status}</span><div className="invoice-actions">{inv.status==="pending"&&<button onClick={()=>cancelInvoiceMutation.mutate(inv.id)} aria-label="Cancel invoice"><Icon name="close" size={16}/></button>}<button onClick={()=>copyInvoiceId(inv)} aria-label="Copy invoice ID" title="Copy invoice ID"><Icon name={copiedId===inv.id?"check":"copy"} size={16}/></button><button onClick={()=>copyInvoice(inv)} aria-label="Copy payment link" title="Copy payment link"><Icon name={copied===inv.id?"check":"link"} size={16}/></button><button onClick={()=>setExpandedId(expandedId===inv.id?null:inv.id)} aria-label={expandedId===inv.id?"Hide ID and link":"Show invoice ID and link"} title={expandedId===inv.id?"Hide ID and link":"Show ID and link"}><Icon name="chevron" size={16} style={{transform:expandedId===inv.id?"rotate(180deg)":"none"}}/></button>{inv.status==="paid"&&<a href={`/api/invoices/${inv.id}/pdf`} download aria-label="Download paid bill PDF" title="Download PDF"><Icon name="receipt" size={16}/></a>}</div></div>{expandedId===inv.id&&<div className="invoice-share-panel"><div style={{display:'flex',alignItems:'center',gap:'8px',padding:'8px 10px',background:'#fff',border:'1px solid var(--line)',borderRadius:'8px',marginBottom:'6px'}}><span style={{fontSize:'10px',fontWeight:800,textTransform:'uppercase',letterSpacing:'0.06em',color:'var(--muted)',flexShrink:0}}>Invoice ID</span><code style={{flex:1,minWidth:0,fontSize:'11px',wordBreak:'break-all'}}>{inv.id}</code><button type="button" onClick={()=>copyInvoiceId(inv)} style={{display:'flex',alignItems:'center',gap:'5px',background:'transparent',border:'1px solid var(--line)',borderRadius:'8px',padding:'5px 9px',fontSize:'11px',fontWeight:700,cursor:'pointer',whiteSpace:'nowrap',flexShrink:0}}><Icon name={copiedId===inv.id?"check":"copy"} size={13}/>{copiedId===inv.id?"Copied":"Copy"}</button></div><div style={{display:'flex',alignItems:'center',gap:'8px',padding:'8px 10px',background:'#fff',border:'1px solid var(--line)',borderRadius:'8px'}}><span style={{fontSize:'10px',fontWeight:800,textTransform:'uppercase',letterSpacing:'0.06em',color:'var(--muted)',flexShrink:0}}>Pay link</span><code style={{flex:1,minWidth:0,fontSize:'11px',wordBreak:'break-all'}}>{invoiceLink(inv)}</code><button type="button" onClick={()=>copyInvoice(inv)} style={{display:'flex',alignItems:'center',gap:'5px',background:'transparent',border:'1px solid var(--line)',borderRadius:'8px',padding:'5px 9px',fontSize:'11px',fontWeight:700,cursor:'pointer',whiteSpace:'nowrap',flexShrink:0}}><Icon name={copied===inv.id?"check":"link"} size={13}/>{copied===inv.id?"Copied":"Copy"}</button></div></div>}</div>)}</div>}</section><DeveloperDashboard /></div>
