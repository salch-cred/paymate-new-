...
const [createdInvoice, setCreatedInvoice] = useState<(Invoice & { payUrl: string }) | null>(null)
 ...
 <section className="panel activity-panel" id="activity"><div className="panel-heading"><div><h2>Invoice activity</h2><p>Live status from creation to verified settlement.</p></div><span className="activity-count">{filteredInvoices.length}</span></div>{invoices.length===0?<div className="activity-empty">Your first invoice will appear here.</div>:filteredInvoices.length===0?<div className="activity-empty">No invoices match &quot;{search}&quot;.</div>:<div className="invoice-table">{filteredInvoices.map(inv=><div className="invoice-row" key={inv.id}><span className={`status-dot ${inv.status}`}/><div className="invoice-row-main"><b>{inv.title}</b><small>{new Date(inv.createdAt).toLocaleDateString()} · {inv.id.slice(0,8)}</small></div><strong>${inv.amountUsd.toLocaleString()}</strong><span className={`status-label ${inv.status}`}>{inv.status}</span><div className="invoice-actions">{inv.status==="pending"&&<button onClick={()=>cancelInvoiceMutation.mutate(inv.id)} aria-label="Cancel invoice"><Icon name="close" size={16}/></button>}<button onClick={()=>copyInvoiceId(inv)} aria-label="Copy invoice ID" title="Copy invoice ID"><Icon name={copiedId===inv.id?"check":"copy"} size={16}/></button><button onClick={()=>copyInvoice(inv)} aria-label="Copy payment link" title="Copy payment link"><Icon name={copied===inv.id?"check":"link"} size={16}/></button>{inv.status==="paid"&&<a href={`/api/invoices/${inv.id}/pdf`} download aria-label="Download paid bill PDF" title="Download PDF"><Icon name="receipt" size={16}/></a>}</div></div>)}</div>}</section><DeveloperDashboard /></div>
