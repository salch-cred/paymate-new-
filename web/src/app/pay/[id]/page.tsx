...
declare global{interface Window{ethereum?:{request:(args:{method:string;params?:unknown[]})=>Promise<unknown>;[key:string]:unknown}}}
...
export default function PayPage({params}:{params:Promise<{id:string}>}){
 ...
 async function handlePay(){if(!isConnected||!address){connect({connector:injected()});return}setStatus("paying");setError(null);try{if(chain?.id!==metisSepolia.id)await switchChainAsync({chainId:metisSepolia.id});const res=await fetch(`/api/pay/${id}/settle`,{method:"POST"});if(res.status!==402){if(res.ok){setStatus("paid");return}throw new Error(`Unexpected settlement status: ${res.status}`)}const requirements=await res.json();const option=requirements.accepts?.[0];if(!option)throw new Error("No valid payment option returned.");if(!window.ethereum)throw new Error("Install an injected Web3 wallet to continue.");const client=createWalletClient({chain:metisSepolia,transport:custom(window.ethereum)});const hash=await client.writeContract({address:(option.token||"0x228B00") as `0x${string}`,abi:[{inputs:[{name:"recipient",type:"address"},{name:"amount",type:"uint256"}],name:"transfer",outputs:[{name:"",type:"bool"}],stateMutability:"nonpayable",type:"function"}],functionName:"transfer",args:[option.payTo as `0x${string}`,parseUnits(option.price.replace("$",""),6)],account:address});const settle=await fetch(`/api/pay/${id}/settle`,{method:"POST",headers:{"Content-Type":"application/json","X-PAYMENT":hash}});if(!settle.ok)throw new Error("Payment verification failed.");setStatus("paid");setInvoice(v=>v?{...v,status:"paid"}:v)}catch(e){setStatus("error");setError(e instanceof Error?e.message:"Payment failed")}}
